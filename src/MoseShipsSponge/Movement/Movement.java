...
public class Movement {
	...
	public static Optional<FailedMovement> move(LiveShip ship, int x, int y, int z, Cause cause, BlockState... movingTo){
		if((x == 0) && (y == 0) && (z == 0)){
			return Optional.of(new FailedMovement(ship, MovementResult.NO_SPEED_SET, 0));
		}
		return move(ship, MovementType.DIRECTIONAL, new StoredMovement.Builder().setX(x).setY(y).setZ(z).build(), cause, movingTo);
	}
	
	public static Optional<FailedMovement> rotateRight(LiveShip ship, Cause cause, BlockState... movingTo){
		return move(ship, MovementType.ROTATE_RIGHT, new StoredMovement.Builder().setRotation(RotateType.RIGHT).build(), cause, movingTo);
	}
	
	public static Optional<FailedMovement> rotateLeft(LiveShip ship, Cause cause, BlockState... movingTo){
		return move(ship, MovementType.ROTATE_LEFT, new StoredMovement.Builder().setRotation(RotateType.LEFT).build(), cause, movingTo);
	}
	
	public static Optional<FailedMovement> rotate(LiveShip ship, RotateType rotate, Cause cause, BlockState... state){
		switch(rotate){
		case LEFT: return rotateLeft(ship, cause, state);
		case RIGHT: return rotateRight(ship, cause, state);
			default: return Optional.of(new FailedMovement(ship, MovementResult.PLUGIN_CANCELLED, true));
		}
	}
	
	public static Optional<FailedMovement> teleport(LiveShip ship, StoredMovement movement, Cause cause, BlockState... movingTo){
		return move(ship, MovementType.TELEPORT, movement, cause, movingTo);
	}
	
	public static Optional<FailedMovement> move(LiveShip ship, MovementType type, StoredMovement movement, Cause cause, BlockState... movingTo){
	...
	private static Optional<FailedMovement> move(LiveShip ship, MovementType type, MovingBlockList blocks, Cause cause){
