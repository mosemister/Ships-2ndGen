...
public class Fixes extends CommandLauncher {
	...
	public void playerCommand(Player player, String[] args) {
		if (args.length == 1) {
			player.sendMessage(ChatColor.GOLD + "/ships fixes teleport" + ChatColor.AQUA
					+ "; repositions the teleporting to ships to your current location");
			player.sendMessage(ChatColor.GOLD + "/ships fixes facing" + ChatColor.AQUA
					+ "; points forward to your looking direction");
		} else {
				...
				if (vessel == null) {
					...
					if (vessel.getOwner().equals(player)) {
						...
						player.sendMessage(Ships
								.runShipsMessage("new teleport location has been set for " + vessel.getName(), false));
					} else {
