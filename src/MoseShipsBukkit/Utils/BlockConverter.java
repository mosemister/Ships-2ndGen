...
public class BlockConverter {
	...
	public static byte convertRotation(MovementMethod move, int id, byte data){
		...
		if ((id == Material.REDSTONE_TORCH_ON.getId()) ||
				(id == Material.REDSTONE_TORCH_OFF.getId()) || 
				(id == Material.TORCH.getId())){
			byte data2 = torchValues(move, data);
			return data2;
		}
		if ((id == Material.WOOD_STAIRS.getId()) || 
				(id == Material.COBBLESTONE_STAIRS.getId()) ||
				(id == Material.ACACIA_STAIRS.getId()) ||
				(id == Material.SANDSTONE_STAIRS.getId()) ||
				(id == Material.BIRCH_WOOD_STAIRS.getId()) ||
