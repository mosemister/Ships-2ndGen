	...
	public boolean execute(Player player, String... args) {
		if (args.length < 5) {
			player.sendMessage(ShipsMain.formatCMDHelp(
					"/ships autopilot l <ship name> <x> <y> <z> : Moves the targeted ship to the co-ords"));
			player.sendMessage(ShipsMain.formatCMDHelp(
			...
			if (player.hasPermission("ships.cmd.autopilot.other")) {
				if (ship.getOwner().isPresent()) {
					...
					if (!owner.getUniqueId().equals(player.getUniqueId())) {
						player.sendMessage(ShipsMain.format("You do not have permission to modify the "
								+ ship.getStatic().getName() + " " + ship.getName(), true));
						return true;
					}
				}
			}
			...
			if (shipA.getAutoPilotData().isPresent()) {
				...
				player.sendMessage(shipA.getName() + " is currently on a diffrent path. Estimated "
						+ (ap.getMovements().size() - ap.getMovesDone()) + " moves left.");
				return true;
			}
			...
			try {
				...
				if (args[1].equalsIgnoreCase("l")) {
					ap = new AutoPilot(shipA, new Location(ship.getLocation().getWorld(), x, y, z).getBlock(),
							ship.getStatic().getDefaultSpeed(), player);
			...
			player.sendMessage(
					ShipsMain.format(args[3] + ", " + args[4] + "and/or " + args[5] + "are not whole numbers", true));
		}
		return false;
	}
